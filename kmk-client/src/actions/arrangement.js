...
export function loadArrangements(filter, sorting) {
    return async (dispatch) => {
        try {
            ...
            const response = await dispatch(apiPost("arrangement/list", request));

            dispatch({ type: SET_ARRANGEMENTS, arrangements: response.payload});
        }
...
export function loadArragement(serialNumber) {
    return async (dispatch) => {
        try {
            const response = await dispatch(apiGet(`arrangement/${serialNumber}`));

            dispatch({ type: SET_ARRANGEMENT, arrangement: response.payload });
        }
        catch(e) {
            console.log(e);
        }
...
export function deleteArrangement() {
    return async (dispatch, getState) => {
        try {
            ...
            await dispatch(apiPost(`arrangement/delete/${arrangement.serialNumber}`));  
        }
        catch(e) {
            console.log(e);
        }
...
export function uploadPart(serialNumber, instrument, file) {
    return async (dispatch) => {
        try {
            ...
            await dispatch(apiPostFormData(`arrangement/upload/${serialNumber}/${instrument}`, form));  
        }
        catch(e) {
            console.log(e);
        }
